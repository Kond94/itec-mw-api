...
const getOrganizationPrinterHistoryUnresolvedProblems = (request, response) => {
  ...
  pool.query(
    "SELECT printers_history.id, printers_history.bw_count, printers_history.color_count, printers_history.total_count, printers_history.problem_description, printers_history.date_reported, printers.name AS printer_name, models.name AS model_name, manufacturers.name AS manufacturer_name, users.name AS resolved_by_name, printer_history_problem_types.name AS problem_type FROM printers_history LEFT JOIN printer_history_problem_types ON printers_history.problem_type = printer_history_problem_types.id LEFT JOIN printers ON printers_history.printer = printers.id LEFT JOIN users ON printers_history.resolved_by = users.id LEFT JOIN models ON printers.model = models.id LEFT JOIN manufacturers ON models.manufacturer = manufacturers.id WHERE resolved = false AND printers.organization = $1 ORDER BY date_reported DESC",
    [id],
    (error, results) => {
      if (error) {
        throw error;
